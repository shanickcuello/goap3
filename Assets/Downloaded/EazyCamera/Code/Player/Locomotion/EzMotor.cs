...
public class EzMotor : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (m_speedDelta == 0) m_speedDelta = .01f;
    }
    public void MovePlayer(float moveX, float moveZ, bool isRunning)
    {
        ...
        m_currentSpeed = Mathf.MoveTowards(m_currentSpeed, isRunning ? m_runSpeed : m_walkSpeed,
            m_acceleration * Time.deltaTime);
        m_moveVector *= m_currentSpeed;
        if (moveX != 0 || moveZ != 0)
        {
            var step = m_rotateSpeed * Time.deltaTime;
            var targetRotation = Quaternion.LookRotation(m_moveVector, Vector3.up);
