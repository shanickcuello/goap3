...
public class EzCameraCollider : MonoBehaviour
{
    ...
    private void Start()
    {
        m_controlledCamera = GetComponent<EzCamera>();
    ...
    {
        if (m_controlledCamera.CollisionsEnabled) HandleCollisions();
    }
    ...
    {
        var nearPlaneCenter = m_cameraTransform.position + m_cameraTransform.forward * m_nearPlaneDistance;
        m_pointBehindCamera = m_cameraTransform.position - m_cameraTransform.forward * m_nearPlaneDistance;
        var halfFOV = Mathf.Deg2Rad * (m_cameraComponent.fieldOfView / 2);
        m_aspectHalfHeight = Mathf.Tan(halfFOV) * m_nearPlaneDistance;
        m_aspectHalfWidth = m_aspectHalfHeight * m_cameraComponent.aspect;
        m_nearClipPlanePoints[0] = nearPlaneCenter +
                                   m_cameraTransform.rotation * new Vector3(-m_aspectHalfWidth, m_aspectHalfHeight);
        m_nearClipPlanePoints[1] = nearPlaneCenter +
                                   m_cameraTransform.rotation * new Vector3(m_aspectHalfWidth, m_aspectHalfHeight);
        m_nearClipPlanePoints[2] = nearPlaneCenter +
                                   m_cameraTransform.rotation * new Vector3(m_aspectHalfWidth, -m_aspectHalfHeight);
        m_nearClipPlanePoints[3] = nearPlaneCenter +
                                   m_cameraTransform.rotation * new Vector3(-m_aspectHalfWidth, -m_aspectHalfHeight);
    }
    ...
    private void CheckCameraPlanePoints()
    {
        ...
        RaycastHit hit;
        float hitDistance = 0;
        for (var i = 0; i < m_nearClipPlanePoints.Length; ++i)
        {
            if (Physics.Linecast(m_controlledCamera.Target.position, m_nearClipPlanePoints[i], out hit, m_layermask))
                if (hit.collider.gameObject.transform.root != m_controlledCamera.Target.root)
                    if (hit.distance > hitDistance)
                    {
                        ...
                        if (!IsOccluded)
                            m_controlledCamera.Settings.ResetPositionDistance =
                                m_controlledCamera.Settings.OffsetDistance;
                        IsOccluded = true;
    ...
    {
        var originalCameraPosition = m_controlledCamera.Target.position +
                                     Vector3.up * m_controlledCamera.Settings.OffsetHeight +
                                     m_cameraTransform.rotation * (Vector3.forward *
                                                                   -m_controlledCamera.Settings.ResetPositionDistance) +
                                     m_cameraTransform.right * m_controlledCamera.Settings.LateralOffset;
        var originalPlaneCenter = originalCameraPosition + m_cameraTransform.forward * m_nearPlaneDistance;
        var halfFOV = Mathf.Deg2Rad * (m_cameraComponent.fieldOfView / 2);
        m_aspectHalfHeight = Mathf.Tan(halfFOV) * m_nearPlaneDistance;
        m_aspectHalfWidth = m_aspectHalfHeight * m_cameraComponent.aspect;
        m_originalClipPlanePoints[0] = originalPlaneCenter +
                                       m_cameraTransform.rotation * new Vector3(-m_aspectHalfWidth, m_aspectHalfHeight);
        m_originalClipPlanePoints[1] = originalPlaneCenter +
                                       m_cameraTransform.rotation * new Vector3(m_aspectHalfWidth, m_aspectHalfHeight);
        m_originalClipPlanePoints[2] = originalPlaneCenter +
                                       m_cameraTransform.rotation * new Vector3(m_aspectHalfWidth, -m_aspectHalfHeight);
        m_originalClipPlanePoints[3] = originalPlaneCenter +
                                       m_cameraTransform.rotation *
                                       new Vector3(-m_aspectHalfWidth, -m_aspectHalfHeight);
        m_pointBehindCamera = m_cameraTransform.position - m_cameraTransform.forward * m_nearPlaneDistance;
    }
    private void CheckOriginalPlanePoints()
    {
        ...
        for (var i = 0; i < m_originalClipPlanePoints.Length; ++i)
        {
#if UNITY_EDITOR
            var lineColor = Color.blue;
#endif
            if (Physics.Linecast(m_controlledCamera.Target.position, m_originalClipPlanePoints[i], out hit,
                    m_layermask))
            {
                lineColor = Color.red;
                objectWasHit = true;
                if (hit.distance < closestHitDistance) closestHitDistance = hit.distance;
            }
