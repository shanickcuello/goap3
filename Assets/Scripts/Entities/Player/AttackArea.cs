...
public class AttackArea : MonoBehaviour
{
    ...
    public float WeaponDmgMult { get => _weaponDmgMult; set => _weaponDmgMult = value; }
    ...
    public void TriggerHit(int dmg, int attackType)
    {
                                                        ...
                                                        x.gameObject.layer == _layerValue))
        {
            ...
            {
                bool killed = opponent.GetHit(_dmg, transform.parent.gameObject, out var exp, out var coins);
    ...
    {
        if (other.gameObject.layer == _layerValue && other.TryGetComponent<IHitable>(out var opponent) && !_brokenWeapon)
        {
            bool killed = opponent.GetHit(_dmg, transform.parent.gameObject, out var exp, out var coins);
