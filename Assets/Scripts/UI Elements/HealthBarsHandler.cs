...
public class HealthBarsHandler : MonoBehaviour
{
    ...
    public static HealthBarsHandler Instance => _instance;
    public Vector2 offset;
    public Slider healthbarPrefab;
    private Camera _camera;
    private Dictionary<Transform, HealthBarData> _entities = new Dictionary<Transform, HealthBarData>();
    private Queue<Transform> _incomingEntities = new Queue<Transform>();
    private Queue<HealthBarData> _incomingData = new Queue<HealthBarData>();
    private Queue<Transform> _outgoingEntities = new Queue<Transform>();
    private void Awake()
    {
    ...
    private void Update()
    {
        if (_incomingEntities.Count > 0 && _incomingData.Count > 0)
        {
            ...
            foreach (var item in incoming) _entities[item.Item1] = item.Item2;
            _incomingEntities.Clear();
            _incomingData.Clear();
        }
        if (_outgoingEntities.Count > 0)
            for (var i = 0; i < _outgoingEntities.Count; i++)
            {
    ...
    {
        if (_entities.ContainsKey(transform)) _outgoingEntities.Enqueue(transform);
    }
}
